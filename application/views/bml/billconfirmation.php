...
function confirmation_page($arguments) {
    log_message('info', 'Inside the bill payment confirmation page answer page =====================================');
    ...
    if (!empty($errors)) {
        ...
        $arguments['text'] = 'Utility Name :' . $beneficiary_name.'
            
            Customer ID Number : '. $ref_code . '
...
Total : ' . $bullet_naira . ' ' . number_format($total, 2, '.', ',');
        ...
        $arguments['cancel_url'] = 'pay_bill/';


        $arguments['confirm_url'] = 'billstatus?refcode=' . urlencode($ref_code) .'&beneficiaryname='.  urlencode($beneficiary_name). '&beneficiary='.urlencode($beneficiary_code).'&pin=' . urlencode($pin) . '&reference=' . urlencode($reference_info) . '&mmoperatorid=' . urlencode($mmoperator) . '&mmoperatorname=' . urlencode($mmoperatorname) . '&amount=' . urlencode($amount);
